...
pub fn get_msg_info(msgid: u24) ?struct {
    ...
    var low: usize = 0;
    var high: usize = MAVLINK_CRC_EXTRA.len - 1;

    while (low <= high) {
        const mid = (low + high) / 2;
        const entry = MAVLINK_CRC_EXTRA[mid];
        const current_msgid = entry[0];

        if (current_msgid == msgid) {
            return .{
                .crc_extra = @intCast(entry[1]),
                .min_payload_len = @intCast(entry[2]),
                .max_payload_len = @intCast(entry[3]),
            };
        } else if (current_msgid < msgid) {
            low = mid + 1;
        } else {
            high = mid - 1;
        }
    ...
    // Message ID not present in the table
    return null;
}
...
pub fn finalize(self: *Self, msgid: u24) u16 {
    ...
    if (msgid < MAVLINK_CRC_EXTRA.len) {
        self.accumulate(get_msg_info(msgid).?.crc_extra);
    }
    return self.crc;
}
...
pub fn crc_accumulate(byte: u8, crc: u16) u16 {
    ...
    tmp ^= tmp << 4;
    return ((crc >> 8) ^ (@as(u16, tmp) << 8) ^ (@as(u16, tmp) << 3) ^ (@as(u16, tmp) >> 4)) & 0xFFFF;
}
...
pub fn crc_accumulate_buf(crc: u16, buf: []const u8) u16 {
    var c = crc;
    for (buf) |b| {
        c = crc_accumulate(b, c);
    }
    return c;
}

const testing = std.testing;
test "get_msg_info returns entry for known msgid" {
    const info = get_msg_info(0);
    try testing.expect(info != null);
    try testing.expectEqual(info.?.crc_extra, 50);
    try testing.expectEqual(info.?.min_payload_len, 9);
    try testing.expectEqual(info.?.max_payload_len, 9);
}

test "get_msg_info returns null for unknown msgid" {
    try testing.expect(get_msg_info(3) == null);
}

test "crc_accumulate_buf matches repeated crc_accumulate" {
    const data = [_]u8{ 1, 2, 3, 4, 5 };
    const initial = crc_init();
    const buf = data[0..];
    const buf_result = crc_accumulate_buf(initial, buf);

    var manual = initial;
    for (buf) |b| manual = crc_accumulate(b, manual);

    try testing.expectEqual(buf_result, manual);
}

test "CRCState.accumulate equals crc_accumulate_buf" {
    const data = [_]u8{ 10, 20, 30 };
    var s = Self.init();
    for (data) |b| s.accumulate(b);
    const state_crc = s.crc;
    const buf_crc = crc_accumulate_buf(crc_init(), data[0..]);
    try testing.expectEqual(state_crc, buf_crc);
}

test "finalize adds the crc_extra byte" {
    const msgid: u24 = 0;
    const extra = get_msg_info(msgid).?.crc_extra;
    var s = Self.init();
    const before = s.crc;
    const after = s.finalize(msgid);
    const manual = crc_accumulate(extra, before);
    try testing.expectEqual(after, manual);
}

test "finalize leaves crc unchanged for out-of-range msgid" {
    const msgid: u24 = @as(u24, @intCast(MAVLINK_CRC_EXTRA.len)) + 1234;
    var s = Self.init();
    const out = s.finalize(msgid);
    try testing.expectEqual(out, crc_init());
}
